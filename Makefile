...
# Best to leave as is
DOCKER_TASKNAME   = piarmy-video-player
...
shell: stop
	...
	@echo ""
	docker run -d --rm --device /dev/vchiq:/dev/vchiq --device /dev/fb0:/dev/fb0 --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME) /bin/ash

run: stop
	docker run -d --rm --device /dev/vchiq:/dev/vchiq --device /dev/fb0:/dev/fb0 --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME)
